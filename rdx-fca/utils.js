...
function formatDeltaMessage(m) {
  ...
  var mentions = m.delta.mentions || {};
  if (m.delta.data && m.delta.data.prng) {
    try {
      const mdata = JSON.parse(m.delta.data.prng);
      for (let i = 0; i < mdata.length; i++) {
        mentions[mdata[i].i] = (m.delta.body || "").substring(mdata[i].o, mdata[i].o + mdata[i].l);
      }
    } catch (e) { }
  }
  // Fallback for new FB mention format in deltas
  if (m.delta.ranges) {
    for (let i = 0; i < m.delta.ranges.length; i++) {
      mentions[m.delta.ranges[i].entity.id] = (m.delta.body || "").substring(m.delta.ranges[i].offset, m.delta.ranges[i].offset + m.delta.ranges[i].length);
    }
  }

  // Name detection fallback: If body contains @Name and mentions is empty
  if (Object.keys(mentions).length === 0 && m.delta.body && m.delta.body.includes('@')) {
    const body = m.delta.body;
    const matches = body.match(/@([^\s@]+)/g);
    if (matches) {
      // Since we don't have the UID mapping here easily, we'll look for tags
      // But usually the bot commands expect the UID. 
      // If Facebook is stripping the UID from metadata, we might be stuck without a lookup.
    }
  }
...
function parseAndCheckLogin(ctx, http, retryCount) {
  ...
  return function (data) {
    function any() {
      if ((data.statusCode >= 500 && data.statusCode < 600) || data.statusCode === 408) {
        if (retryCount >= 5) {
        ...
        retryCount++;
        const retryTime = Math.floor(Math.random() * 5000) + 2000;
...
function CustomError(errorData) {
  this.name = "CustomError";
  this.message = (errorData && (errorData.error || errorData.errorSummary)) ? (errorData.error || errorData.errorSummary) : JSON.stringify(errorData);
...
module.exports = {
  ...
  capture: function (name, args) {
    // Capture logs disabled as requested
    // const timestamp = new Date().toLocaleTimeString("en-PK", { timeZone: "Asia/Karachi" });
    // console.log(chalk.blue(`[${timestamp}] [CAPTURE] [${name}]`), JSON.stringify(args, null, 2));
  },
  // No Part of Conversation Fix
  capture: function (name, args) {
    // Capture logs disabled as requested
    // const timestamp = new Date().toLocaleTimeString("en-PK", { timeZone: "Asia/Karachi" });
    // console.log(chalk.blue(`[${timestamp}] [CAPTURE] [${name}]`), JSON.stringify(args, null, 2));
