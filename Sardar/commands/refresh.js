...
// Fetch ALL group threads from Facebook where bot is still a member
...
module.exports = {
  ...
  async run({ api, event, send, Threads , config }) {
    ...
    // ── Phase 2: Fetch ALL groups from Facebook (only where bot is a member) ──
    const botID = api.getCurrentUserID();
    let fbGroups = [];
    try { fbGroups = await fetchAllGroupThreads(api, botID); } catch {}
    clearInterval(anim);
